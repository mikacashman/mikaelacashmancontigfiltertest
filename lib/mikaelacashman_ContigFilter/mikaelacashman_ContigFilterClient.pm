package mikaelacashman_ContigFilter::mikaelacashman_ContigFilterClient;
...
mikaelacashman_ContigFilter::mikaelacashman_ContigFilterClient

=head1 DESCRIPTION


A KBase module: mikaelacashman_ContigFilter
...
sub new
{
    ...
    my $self = {
	client => mikaelacashman_ContigFilter::mikaelacashman_ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a mikaelacashman_ContigFilter.FilterContigsParams
$return is a mikaelacashman_ContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a mikaelacashman_ContigFilter.workspace_name
	contigset_id has a value which is a mikaelacashman_ContigFilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a mikaelacashman_ContigFilter.ws_contigset_id
...
# Authentication: required
	...
	method => "mikaelacashman_ContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$params is a mikaelacashman_ContigFilter.FilterContigsMaxParams
$return is a mikaelacashman_ContigFilter.FilterContigsResults
...
# Authentication: required
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "mikaelacashman_ContigFilter.filter_contigs_max",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "mikaelacashman_ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for mikaelacashman_ContigFilter::mikaelacashman_ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "mikaelacashman_ContigFilter::mikaelacashman_ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package mikaelacashman_ContigFilter::mikaelacashman_ContigFilterClient::RpcClient;
